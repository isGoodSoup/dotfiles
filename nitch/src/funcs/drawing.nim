...
proc drawInfo*(asciiArt: bool) =
  ...
  let ramUsed = ramInfo.split('|')[0].strip()
  let ramMiB = parseFloat(ramUsed)
  let ramGB  = $(round(ramMiB / 1024 * 100) / 100)
  ...
  # colored out
    ...
    stdout.styledWrite("  │ ", color2, ramIcon, color0, ramCat, fgYellow, ramGB & " GB", color0, "\n")
    stdout.styledWrite("  ├───────────┤\n")
